...
class DeletionRegistry with DeletionRegistryMappable {
  ...
  bool cleanRegistry({
    ...
    final shouldInvalidateCache = getLastSyncedAt(deviceId: deviceId) == null;
    ...
    deletionsByCollection.forEach((collectionId, doc) {
      doc.forEach((docId, deviceIds) {
        if (deviceIds.containsAll(allDeviceIds)) {
          for (var deviceId in deviceIds) {
            deletions[deviceId]?[collectionId]?.remove(docId);
          }
        }
      });
    });
